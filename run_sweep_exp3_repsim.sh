...
#SBATCH --job-name=ti-repsim3
#SBATCH --gres=gpu:1
#SBATCH --time=4:00:00
#SBATCH --mem=40G
#SBATCH --cpus-per-task=4
#SBATCH --array=0-39
#SBATCH --output=logs/repsim3_%A_%a.out
#SBATCH --error=logs/repsim3_%A_%a.err
...
# ── Decode array index ─────────────────────────────────────────────────────────
# No L2 dimension: 4 models × 10 (task, hierarchy) configs = 40 array elements.
...
echo "  model=${MODEL}  task=${TASK_SLUG}  hier=${HIER_TAG}"
...
for INNER_ID in $(seq 0 99); do
    ...
    OUTPUT_DIR="results/sweep3_repsim/${MODEL_SLUG}/${TASK_SLUG}/${HIER_TAG}/seed${SEED}_${REV_TAG}"

    if [ -f "${OUTPUT_DIR}/rep_sim_pre.npy" ]; then
